...
music()
{
	...
	if [ ! -z "$m_symbol_" ] ; then
		music_=$(mpc | sed "/^volume:/d;s/\\&/&amp;/g;/\\[paused\\].*/d;/\\[playing\\].*/d;/^ERROR/Q" | paste -sd ' ' -;)

		if [ ${#music_} -gt 42 ] ; then
			music_="『"$(echo $music_|cut -c1-42)...』" $m_symbol_ "
		else
			music_="『"$music_"』"
		fi
...
pkg_updates() {
  ...
  if [ -z "$updates" ]; then
    ...
    printf "^c#7797b7^ $updates"" updates"
  fi
  # if [ -z "$updates" ]; then
